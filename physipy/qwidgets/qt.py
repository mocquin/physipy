...
class QuantityQtSlider(QWidget):
    ...
    def __init__(self, qminimum, qmaximum, value=None, descr="Quantity", favunit=None, parent=None):
        ...
        self.cb.addItems([u_str for u_str, u_q in self.units.items() if qminimum.dimension == u_q.dimension])
        ...
        self.qlineedit.setText(self.text_value) # Programmatically sets the text
    ...
    def raw_to_public(self, raw_value):
        try:
            public_value = self.qminimum + float(raw_value) * (self.qmaximum - self.qminimum) / (self.qtslider.maximum() - self.qtslider.minimum())
            return public_value
        except:
            pass
    ...
    def public_to_raw(self, public_value):
        try:
            raw_value = (public_value - self.qminimum)/( self.qmaximum - self.qminimum)* (self.qtslider.maximum() - self.qtslider.minimum())
            return raw_value
        except:
            pass
