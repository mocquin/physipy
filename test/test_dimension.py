...
class TestDimension(unittest.TestCase):
    ...
    def test_110_siunit_dict(self):
        ...
        self.assertEqual(
            Dimension({"J": -1, "L": 1, "theta": 3}).siunit_dict(),
            {
                "m": 1,
                "kg": 0,
                "s": 0,
    ...
    def test_repr_latex(self):
        self.assertEqual(Dimension(None)._repr_latex_(), "$1$")
        self.assertEqual(Dimension({"L": 1})._repr_latex_(), "$L$")
        self.assertEqual(Dimension({"L": 2})._repr_latex_(), "$L^{2}$")
        self.assertEqual(
            Dimension({"J": -1, "L": 1, "theta": 3})._repr_latex_(),
            r"$\frac{L \theta^{3}}{J}$",
        )

    def test_latex_SI_unit(self):
        self.assertEqual(Dimension(None).latex_SI_unit(), "$1$")
        self.assertEqual(Dimension({"L": 1}).latex_SI_unit(), "$m$")
        self.assertEqual(Dimension({"L": 2}).latex_SI_unit(), "$m^{2}$")
        self.assertEqual(
            Dimension({"J": -1, "L": 1, "theta": 3}).latex_SI_unit(),
            r"$\frac{K^{3} m}{cd}$",
        )
